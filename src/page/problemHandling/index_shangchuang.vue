<template>
  <div class="main">
    ...
    <Modal v-model="showtreatflag" width="400" height="400" :styles="{top: atuomodeltop}" @on-ok="handle">
      ...
      <Upload ref="Upload" multiple :on-success="handleSuccess" :before-upload="handleUpload" :action=" baseUrl + 'updateRiver_i'">
            ...
            return h('div', [
              ...
              h('Button', {
               ...
               h('Button', {
                props: {
                  type: 'info',
                  disabled: Cookies.get('type') == '7' && params.row.supervise == 0 ? false : true
                },
                style: {
                  marginLeft: '15px',
                  display: Cookies.get('type') != '7' && params.row.supervise == 0 ? 'none' : 'inline-block',
                  color: 'red'
                },
                on: {
                  click: (e) => {
  ...
  methods: {
    ...
    handle() {
      ...
      if (Cookies.get('type') != '5') { //河长办
        ...
        formData.append("type", this.problemtype);
